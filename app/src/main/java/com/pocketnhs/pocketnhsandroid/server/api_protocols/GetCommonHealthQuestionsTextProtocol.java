...
public class GetCommonHealthQuestionsTextProtocol implements PocketNHSServerProtocol {

    //private static final String URL_ENDPOINT =  ServerSettings.SERVER_BASE_URL +  "/chq/articles";

    private static final String PROTOCOL_ID =  "CHQ Question Text";
    ...
    public String getEndpointURL(Map<String, Object> params) {
        ...
        String endpoint =  removeKey((String)params.get(DETAILS_LINK));
        endpoint = endpoint + ".xml"
                + "?" + ServerSettings.API_KEY_STR;
        return endpoint;
    }

    private String removeKey(String strWithKey) {
        String strNoKey = strWithKey.substring(0,strWithKey.indexOf("?"));
        return  strNoKey;
    }

    public static synchronized PocketNHSServerProtocol getInstance() {
        if (mInstance == null) {
            mInstance = new GetCommonHealthQuestionsTextProtocol();
        }
        return mInstance;
    }
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.END_TAG:
                        ...
                        if (name.equalsIgnoreCase(RESPONSE_LINK_ELEMENT)
                                && textLinkPair != null) {
                            if (isArticle(textLinkPair.mLink)) {
    ...
    private boolean isArticle(String mLink) {
        if (mLink.contains("chq/articles")){
            return true;
        }else{
            return false;
        }
